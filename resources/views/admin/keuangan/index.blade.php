        ...
        <div class="col">
            <div class="card">
                <div class="card-header">
                    <a href="{{ route('admin.keuangan.create') }}" class="btn btn-primary btn-sm">Tambah Data</a>
                </div>
                <div class="card-body table-responsive">
                    <table id="dataTable1" class="table table-bordered table-hover">
                        ...
                        <tbody>
                            ...
                            @foreach ($keuangan as $js)
                                <tr>
                                    <td>{{ $no++ }}</td>
                                    <td>{{ $js->tanggal }}</td>
                                    <td>{{ $js->uang_keluar }}</td>
                                    <td>{{ $js->saldo_akhir }}</td>
                                    <td>{{ $js->keterangan }}</td>
                                    <td>
                                        <div class="row ml-2">
                                            {{-- <a href="{{ route('admin.keuangan.edit', $js->id) }}"
                                                class="btn btn-primary btn-sm"><i class="fas fa-edit fa-fw"></i></a> --}}
                                            <form method="POST" action="{{ route('admin.keuangan.destroy', $js->id) }}">
                                                @csrf
                                                @method('DELETE')
